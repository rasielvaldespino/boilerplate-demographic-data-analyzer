...
def calculate_demographic_data(print_data=True):
    ...
    # 1. Número de cada raza representada
    race_count = df['race'].value_counts()

    # 2. Edad promedio de los hombres
    average_age_men = round(df[df['sex'] == 'Male']['age'].mean(), 1)

    # 3. Porcentaje de personas con licenciatura (Bachelors)
    ...
    # % con educación avanzada y salario >50K
    higher_education_rich = round(
        (df[higher_education]['salary'] == '>50K').mean() * 100, 1
    )

    # % sin educación avanzada y salario >50K
    lower_education_rich = round(
        (df[lower_education]['salary'] == '>50K').mean() * 100, 1
    )

    # 5. Horas mínimas trabajadas
    min_work_hours = df['hours-per-week'].min()

    # % de personas que trabajan mínimas horas y ganan >50K
    num_min_workers = df[df['hours-per-week'] == min_work_hours]
    rich_percentage = round(
        (num_min_workers['salary'] == '>50K').mean() * 100, 1
    )

    # 6. País con mayor % de personas que ganan >50K
    ...
    country_salary_over_50k = country_salary['>50K'] * 100
    ...
    # 7. Ocupación más popular para >50K en India
